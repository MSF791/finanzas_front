...
export function Gastos({ volver, editar }) {
  ...
  useEffect(() => {
    async function LoadAllGastos() {
      ...
      const GastoUsuario = respuesta.data.filter(gasto => gasto.usuario === id);
  ...
  const handleDeleteGasto = (idGasto) => {
    setGastos(prevGastos => prevGastos.filter(gasto => gasto.id !== idGasto));
  };

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 ml-6">
      {gastos.map((gasto) => (
        <CardGastos key={gasto.id} gasto={gasto} onDelete={handleDeleteGasto} editar={editar}/>
      ))}
    </div>
  );
}
